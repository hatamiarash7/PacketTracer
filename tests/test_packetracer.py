...
from packetracer import packetracer, checksum, triggerlist
from packetracer.psocket import SocketHndl
import packetracer.ppcap as ppcap
import packetracer.pcapng as pcapng
from packetracer import statemachine
from packetracer import lazydict
from packetracer.layer12 import aoe, arp, btle, can, dtp, ethernet, ieee80211, linuxcc, ppp, radiotap, stp, vrrp,\
	flow_control, lldp, slac
from packetracer.layer3 import ip, ip6, ipx, icmp, igmp, ospf, pim
from packetracer.layer4 import tcp, udp, ssl, sctp
from packetracer.layer567 import diameter, dhcp, dns, der, hsrp, http, mqtt, ntp, pmap, radius, rip, rtp, someip,\
	telnet, tpkt
...
class MyPacket(packetracer.Packet):
	pass
...
class GeneralTestCase(unittest.TestCase):
	...
	def test_dissectfail(self):
		...
		class PktTlist(packetracer.Packet):
			__hdr__ = (
				("field1", "H", 0x0),
...
class EthTestCase(unittest.TestCase):
	def test_eth(self):
		...
		self.assertTrue(eth1.is_direction(eth1, packetracer.Packet.DIR_SAME))
...
class IPTestCase(unittest.TestCase):
	def test_IP(self):
		...
		self.assertEqual(ip1.direction(ip1), packetracer.Packet.DIR_SAME)
...
class TCPTestCase(unittest.TestCase):
	def test_TCP(self):
		...
		self.assertTrue(tcp1.is_direction(tcp2, packetracer.Packet.DIR_REV))
...
class UDPTestCase(unittest.TestCase):
	def test_UDP(self):
		...
		self.assertTrue(udp1.is_direction(udp2, packetracer.Packet.DIR_SAME))
		# checksum
...
class IP6TestCase(unittest.TestCase):
	def test_IP6(self):
		...
		self.assertEqual(pkt_eth_ip_tcp.higher_layer.nxt,
			packetracer.Packet.get_id_for_handlerclass(pkt_eth_ip_tcp.higher_layer.__class__,
...
class StaticsTestCase(unittest.TestCase):
	def test_dns(self):
		dns_string = "www.test1.test2.de."
		dns_bytes = b"\x03www\x05test1\x05test2\x02de\x00"
		self.assertEqual(dns_string, packetracer.dns_name_decode(dns_bytes))
		self.assertEqual(dns_bytes, packetracer.dns_name_encode(dns_string))
...
if len(sys.argv) == 1:
	...
	# python tests/test_packetracer.py TestClass
